...
```bash
conda create python=3.8 -n notes_app
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate notes_app
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///notes_app` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE notes_app WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE notes_app TO ;
```
...
ERROR:  database "notes_app" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists notes_app
